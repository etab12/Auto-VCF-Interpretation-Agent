"""Analyst agent: VCF validation + variant QC + VEP annotation.
 
Takes the input .vcf.gz, checks it is structurally usable, applies the QC
rules in skills/qc.md, then annotates through the Ensembl VEP REST API
following skills/annotation.md.
...
def build_validation_task(context=None, output_file=None):
    """Structural validation. Gate for everything downstream."""
    return Task(
        description=(
            "Validate the input VCF file before any analysis: {file_path}\n\n"
            "1. Call the validate_vcf_file tool with vcf_path={file_path}\n"
            "2. Report the status the tool returned, together with every "
            "error and warning it listed. Quote them, do not paraphrase.\n"
            "3. Summarise the structural details the tool reported: VCF "
            "version, number of variant records, number of samples, number "
            "of malformed lines, chromosome naming style and how many "
            "distinct chromosomes are present.\n"
            "4. State clearly whether analysis may continue. Status INVALID "
            "means it may not.\n\n"
            "Report only what the tool returned. Do not open, guess at or "
            "describe the contents of the file yourself."
        ),
        expected_output=(
            "A validation report containing:\n"
            "- The file path checked\n"
            "- STATUS: VALID | VALID_WITH_WARNINGS | INVALID\n"
            "- Every error and warning reported by the tool, quoted verbatim\n"
            "- Structural details: VCF version, record count, sample count, "
            "malformed line count, chromosome naming style, distinct "
            "chromosome count\n"
            "- A final line stating whether QC and annotation may proceed"
        ),
